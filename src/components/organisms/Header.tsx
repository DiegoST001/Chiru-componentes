...
function Header({ className, showSearch = true }: HeaderProps) {
  ...
  // Fallback for token if not provided by context
  ...
  // Si hay token, mostrar userName y userAbbreviation aunque contactName esté vacío
  const hasToken = Boolean(token);
  const hasUserName = (user as any)?.userName && (user as any)?.userName.trim().length > 0;
  const hasUserAbbr = (user as any)?.userAbbreviation && (user as any)?.userAbbreviation.trim().length > 0;
  // Unificar lógica de displayName e iniciales como en UserProfileSidebar
  let effective: any = user ?? undefined;
  if (!effective) {
    try {
      if (typeof window !== "undefined") {
        const raw = localStorage.getItem(STORAGE_KEYS.USER_SESSION);
  ...
  const isLogged = Boolean(isAuthenticated) || Boolean(token);
